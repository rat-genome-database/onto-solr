...
(
select wm_concat(unique concat(' ', concat(o1.term_acc, concat('; ', o1.term))))  from ont_dag, ont_terms o1
where o1.ont_id not in('EFO','CVCL') 
...
on
t.TERM_ACC = s.acc
where t.IS_OBSOLETE = 0 and  t.ont_id not in('EFO','CVCL')
order by t.term_acc

...
./post_to_onto_solr.sh delete_all.xml
if [ $? -ne 0 ]; then
...
./post_to_onto_solr.sh /data/OntoSolr_data/onto_solr.xml
if [ $? -ne 0 ]; then
...
./post_to_onto_solr.sh custom_maps.xml
if [ $? -ne 0 ]; then
...
./post_to_onto_solr.sh /data/OntoSolr_data/onto_solr_genes.xml
if [ $? -ne 0 ]; then
...
./post_to_onto_solr.sh /data/OntoSolr_data/XP_solr.xml
if [ $? -ne 0 ]; then
...
./post_to_onto_solr.sh del_redundant_XP.xml
if [ $? -ne 0 ]; then
  echo "ERROR: delete redundant XP terms in OntoSolr failed!"
  exit 1
fi

#echo "  --OntoSolr: uploading UMLS concepts"
#./post_to_onto_solr.sh /data/OntoSolr_data/umls_concepts.xml
